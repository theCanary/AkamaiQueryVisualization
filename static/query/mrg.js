...
$(function () {
    ...
    $("<p><select id ='option' class='form-control'><option>Number of Rows Merged</option> <option>Number of Contributors</option><option>Elapsed Time (ms)</option></select> </p>").appendTo('#option_inputs');
...
var submit_query = function(e) {
  var optionNum = ["Number of Rows Merged", "Number of Contributors", "Elapsed Time (ms)"].indexOf($('#option').val());
  var page_args = {
    table: 'mrg',
    a: $('#spanNames').val()[0],
    b: $('#domainNames').val()[0],
  ...
  var mrgws = open_websocket("/socket/");
  mrgws.onmessage = function(event) {
        ...
        if (data.table == 'mrg') {
          ...
          console.log(data.span, page_args['a'], data.domain, page_args['b'], data.tablename, page_args['c'])
          if ((data.span == page_args['a'] ||  page_args['a']=="ALL") &&
              (data.domain == page_args['b'] ||  page_args['b']=="ALL") &&
              (data.tablename == page_args['c'] ||  page_args['c']=="")) {
            point = data.data[optionNum];
            console.log("point will be added")
